	...
	patas   {
		...
		},
		{
			new PataAraxxor(
				...
				{-30.0f, 46.1f, 0-13.0f}, {1.0f, 0, -1.0f},
...
void Araxxor :: AplicarTextura  (Textura * nueva, const bool calcular) noexcept
{
	...
	for (unsigned char i = 0; i < PATAS; i++)
	{
		patas[derecha][i]->AplicarTextura(nueva, calcular);
		patas[izquierda][i]->AplicarTextura(nueva, calcular);
	}
}
...
) noexcept
{
	...
	for (unsigned char i = 0; i < PATAS; i++)
	{
		patas[derecha][i]->Dibujar(dibujado, ajedrez, color, seleccion);
		patas[izquierda][i]->Dibujar(dibujado, ajedrez, color, seleccion);
	}
}
