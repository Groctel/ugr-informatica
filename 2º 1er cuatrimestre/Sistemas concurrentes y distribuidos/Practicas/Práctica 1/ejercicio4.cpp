...
const int ESTANQUEROS = 10;
const int FUMADORES = 10;
const int ESPACIO_MOSTRADOR = 4;

vector<Semaphore> estanqueros;
vector<Semaphore> ingredientes;
vector<Semaphore> espacio_mostrador;

unsigned mostrador[FUMADORES] = {ESPACIO_MOSTRADOR};
...
void funcion_hebra_estanquero(int indice ) {
	while (true) {
      ...
      sem_wait (espacio_mostrador[ingrediente]);
...
void  funcion_hebra_fumador( int num_fumador )
{
   while(true) {
		...
		mostrador[num_fumador] ++;
		sem_signal (espacio_mostrador[num_fumador]);
...
int main() {
	...
	for (int i=0; i<FUMADORES; i++) {
		...
		espacio_mostrador.push_back(ESPACIO_MOSTRADOR);
	}
