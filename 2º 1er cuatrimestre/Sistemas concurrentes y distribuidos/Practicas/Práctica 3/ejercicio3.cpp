 ...
 */
	for (unsigned i=0; i<num_items; i++) {
		...
		if (dormir >= 46) {
			...
			MPI_Recv(&valor_prod, 1, MPI_INT, id_dormitorio, etiq_despertar,
...
void funcion_dormitorio () {
	...
	while (true) {
		MPI_Recv(&peticion, 1, MPI_INT, MPI_ANY_SOURCE, etiq_dormir,
		         MPI_COMM_WORLD, &estado);
		...
		MPI_Ssend(&peticion, 1, MPI_INT, estado.MPI_SOURCE, etiq_despertar,
		          MPI_COMM_WORLD);
	}
}
