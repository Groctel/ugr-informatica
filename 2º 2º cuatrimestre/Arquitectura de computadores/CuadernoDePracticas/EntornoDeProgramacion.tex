...
Ejecutar \code{lscpu} en el PC y en un nodo de cómputo de \textit{atcgrid}.
...
Ejecución de \code{lscpu} en el PC:
...
Ejecución de \code{lscpu} en el \textit{atcgrid}:
...
Compilar y ejecutar en el PC el código \code{HelloOMP.c} del seminario.

\begin{itemize}
	\item Mostar la compilación y ejecución en el PC\@.
	\item Justificar el número de ``Hello world'' que se imprimen en pantalla teniendo en cuenta la salida que devuelve \code{lscpu}.
\end{itemize}
...
Se imprimen $12$ ``Hello World'', uno por cada hebra (o núcleo lógico) del sistema identificado por un número en el rango $[0-11]$ mediante la llamada \code{omp\_get\_thread\_num()}.

\section{}\label{ej1-3}

\subsubsection{Enunciado}

Copiar el ejecutable de \code{HelloOMP.c} que ha generado en~\ref{ej1-2} y que se encuentra en su directorio del PC al directorio de su \code{\$HOME} en el \textit{front-end} de \textit{atcgrid}.
Ejecutar este código en un nodo de cómputo de \textit{atcgrid} a través de cola \code{ac} del gestor de colas (no use ningún script) utilizando directamente en la línea de comandos:
...
¿Qué orden \code{srun} usaría para que \code{HelloOMP} utilice los $12$ núcleos físicos de un nodo de cómputo de \textit{atcgrid} (se debe imprimir un único mensaje desde cada uno de ellos, $12$ en total)?
...
Para que \code{HelloOMP} utilize los $12$ núcleos físicos de un nodo de cómputo de \textit{atcgrid} se debe usar la siguiente orden:
...
Modificar en su PC \code{HelloOMP.c} para que se imprima ``world'' en un \code{printf} distinto al usado para ``Hello''.
En ambos \code{printf} se debe imprimir el identificador de la hebra que escribe en pantalla.
Nombrar al código resultante \code{HelloOMP2.c}.
...
Ejecutar el código en un nodo de cómputo de \textit{atcgrid} usando el script \code{script\_helloomp.sh} del seminario.

Utilizar la siguiente orden\footnote{%
	Utilizar siempre con \code{sbatch} las opciones \code{-n1}, \code{--cpus-per-task}, \code{--exclusive} y, para usar núcleos físicos y no lógicos, no olvidar incluir \code{--hint=nomultithread}.
	Utilizar siempre con \code{srun}.
	Si lo usa fuera de un script, usar las mismas opcioens que con \code{sbatch} a excepción de \code{--exclusive}.
	Recordar que los \code{srun} dentro de un script heredan las opciones utilizadas en el \code{sbatch} que se usa para enviar el script a la cola \code{slurm}.
	Se recomienda usar \code{sbatch} en lugar de \code{srun} para enviar trabajos a ejecutar a través \code{slurm} porque éste último deja bloqueada la ventana hasta que termina la ejecución, mientras que usando \code{sbatch} la ejecución se realiza en segundo plano.
}:
...
\code{HelloOMP2.c}:
\lstinputlisting[language=C]{CuadernoDePracticas/Ej1.4/HelloOMP2.c}
...
El script se ha ejecutado por el nodo \code{atcgrid1}, como indica la línea \code{Nodos asignados al trabajo}.
...
Generar en el PC el ejecutable del código fuente C del \textit{Listado 1} para vectores locales (para ello, antes de compilar, debe descomentar la definición de \code{VECTOR\_LOCAL} y comentar las definiciones de \code{VECTOR\_GLOBAL} y \code{VECTOR\_DYNAMIC}).
El comentario inicial del código muestra la orden para compilar (siempre hay que usar \code{-O2} al compilar como se indica en las normas de prácticas).
...
\subsubsection{Enunciado}
En el código del \textit{Listado 1} se utiliza la función \code{clock\_gettime()} para obtener el tiempo de ejecución del trozo de código que calcula la suma de vectores.
El código se imprime la variable \code{ncgt}.
¿Qué contiene esta variable?

¿En qué estructura de datos devuelve \code{clock\_gettime()} la información de tiempo?
...
La variable \code{ncgt} contiene el tiempo que tarda el programa en realizar los cómputos con los tres vectores.

La llamada a \code{clock\_gettime()} devuelve el tiempo en una estructura \code{timespec}, que es una estructura definida en \code{<time.h>} y que está compuesta por un dato \code{time\_t} que contiene el número de segundos desde el 1 de enero de 1970 y un dato \code{long} que contiene el número de nanosegundos pasados desde el último segundo.
...
\end{tabular}
\caption{Resultados de \code{SumaVectores.c} en mi PC}
\end{table}
...
\end{tabular}
\caption{Resultados de \code{SumaVectores.c} en el \textit{atcgrid}}
\end{table}
\end{center}
...
¿Cuál es el máximo valor que se puede almacenar en la variable \code{N} teniendo en cuenta su tipo?
Razonar la respuesta.

Modificar el código fuente C (en el PC) para que el límite de los vectores cuando se declaran como variables globales sea igual al máximo número que se puede almacenar en la variable \code{N} y generar el ejecutable.
¿Qué ocurre?
¿A qué es debido?
...
El máximo valor almacenable en \code{N} es $2^{32}-1 = 4294967295$, definido en la constante \code{UINT\_MAX} de \code{<limits.h>}.
Es el valor máximo almacenable en un \code{unsigned} en x86\_64.
