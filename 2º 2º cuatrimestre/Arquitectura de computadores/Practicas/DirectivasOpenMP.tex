...
El primer elemento, \code{\#pragma omp}, indica al compilador que la línea contiene una \code{directiva} OpenMP\@.
...
También podemos modificar el número de hebras que ejecutan el código mediante la variable de entorno \code{OMP\_NUM\_THREADS}\footnote{%
	A lo largo del resto de los apuntes se deja el valor de \code{OMP\_NUM\_THREADS} a $4$ para simplificar la muestra de la salida estándar.
}.
...
La combinación de directivas acepta todas las cláusulas de ambas directivas combinadas excepto \code{nowait}, cuyo funcionamiento no forma parte del contenido de estos apuntes.
