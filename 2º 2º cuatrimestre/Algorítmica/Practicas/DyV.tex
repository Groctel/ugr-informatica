...
\begin{itemize}
	\item\code{unsigned* pre}\textbf{:} El vector en preorden.
	\item\code{unsigned raiz}\textbf{:} La posición de inicio del vector en preorden.
	\item\code{unsigned tope}\textbf{:} La posición siguiente al fin del vector en preorden.
	\item\code{bool** matriz}\textbf{:} La matriz de adyacencia a rellenar.
\end{itemize}
...
Este primer algoritmo contiene un bucle \code{for} externo con inicialización, comprobación y actualización de eficiencia constante, que se ejecuta de $tope-raiz$ veces, por lo que su eficiencia es $O(n)$.
En su interior tenemos un bloque \code{if}, cuya comprobación y cuerpo se ejecutan todas en tiempo constante, por lo que su eficiencia total es $O(1)$.
Contra este bloque \code{if} tenemos un bloque \code{else if} de comprobación en tiempo constante que contiene un bucle \code{for}.
...
Para este algoritmo contamos con un bucle \code{for} sin cuerpo y con inicialización, comprobación y actualización de eficiencia constante que se ejecuta como máximo $tope-raiz$ veces, de forma que su eficiencia es $O(n)$.
Tras este bucle, encontramos un bloque \code{if} de comprobación constante y cuerpo con dos llamadas recursivas y resto constante y un bloque \code{else if} de iguales características que el bloque \code{if} pero con una sola llamada recursiva, por lo que tomamos el bloque \code{if} para analizar la eficiencia.
