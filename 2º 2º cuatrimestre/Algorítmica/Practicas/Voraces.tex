...
Mediante una búsqueda en profundidad (\code{dfs}) y usando una lista de abiertos (\code{color[v] = gray}) y cerrados (\code{color[v] = black}), la autora resuelve el problema mediante técnicas de inteligencia artificial y recursividad.
...
Esta implementación comienza con un nodo \code{start}, que se selecciona de la siguiente forma:
...
Esta función devuelve busca los vértices adyacentes al vértice \code{v}, que es el vértice candidato para formar la siguiente arista del circuito, de forma que devuelve \code{true} si la única arista que parte de dicho vértice es la que lo conecta con el vértice \code{u}.

\section{Implementación del algoritmo}\label{voraces-implementacion}

Teniendo en cuenta las implementaciones presentadas en el apartado anterior, definimos una clase \code{Grafo} con los siguientes prototipos:
...
\begin{itemize}
	\item\textbf{Sustituir el array dinámico de vértices por un \code{std::vector}:} De esta forma, el número de vértices del grafo no queda determinado en el momento de su construcción y su estructura es modificable sin ser necesario modificar los parámetros de construcción.
	\item\textbf{Modificar el \code{std::list} de vértices adyacentes por un \code{std::set}:} De esta forma, nos aseguramos de que el grafo no sea un multigrafo\footnote{%
...
\begin{itemize}
	\item Recibir los vértices que forman la arista y un vector de vértices visitados inicializado a \code{false}.
...
Por tanto, definimos la función \code{EsPuente} de la siguiente forma:
...
Comenzamos con la búsqueda en profundidad, que contiene un bucle \code{for} que recorre todos los vértices adyacentes al vértice que recibe secuencialmente y se llama a sí mismo si encuentra un vértice no visitado por las llamadas ancestras.
...
Esta búsqueda en profundidad es accedida dos veces desde \code{EsPuente}, que contiene dos bucles \code{for} de eficiencia $O(n)$ y la llama dos veces, por lo que \code{EsPuente} también tiene eficiencia $O(n!)$.
Por último, llamamos a \code{EsPuente} en \code{ImprimeElemento} que, aparte de esta llamada, sólo contiene operaciones elementales y una llamada recursiva.
Como en el caso peor estamos trabajando en un grafo $K_n$, sabemos que éste se va a ejecutar $n$ veces y va a llamar a \code{EsPuente} para que opere con $n-1$ vértices.
